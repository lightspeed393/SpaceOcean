...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allowZAddresses)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SPACE address (e.g. %1)").arg(QString("RQziYMaozbM5sVdYqotjVKdZZWxtrGkei9")));
#endif
...
bool parseKomodoURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("SPACE"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!KomodoUnits::parse(KomodoUnits::SPACE, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseKomodoURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("SPACE://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "SPACE:");
    }
...
{
    QString ret = QString("SPACE:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(KomodoUnits::format(KomodoUnits::SPACE, info.amount, false, KomodoUnits::separatorNever));
        paramCount++;
    }
