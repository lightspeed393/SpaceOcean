...
CBlockTemplate* CreateNewBlock(CPubKey _pk,const CScript& _scriptPubKeyIn, int32_t gpucount, bool isStake)
{
    ...
    CBlockIndex* pindexPrev = 0;
    {
             ...
             mi != mempool.mapTx.end(); ++mi)
        {
            ...
            {
                //LogPrintf("CreateNewBlock: komodo_validate_interest failure nHeight.%d nTime.%u vs locktime.%u\n",nHeight,(uint32_t)pblock->nTime,(uint32_t)tx.nLockTime);
                continue;
            }
        ...
        if ( ASSETCHAINS_SYMBOL[0] != 0 && isStake )
        {
            ...
            else
            {
                ...
                while ( blocktime-57 > GetAdjustedTime() )
                {
                    sleep(1);
                    if ( (rand() % 100) < 1 )
                        LogPrintf( "%u seconds until elegible, waiting.\n", blocktime-((uint32_t)GetAdjustedTime()+57));
                    if ( chainActive.LastTip()->GetHeight() >= stakeHeight )
                    {
                        LogPrintf( "Block Arrived, reset staking loop.\n");
                        return(0);
                    }
            ...
            if ( siglen > 0 )
            {
                ...
                //LogPrintf("staking PoS ht.%d t%u lag.%u\n",(int32_t)chainActive.LastTip()->GetHeight()+1,blocktime,(uint32_t)(GetAdjustedTime() - (blocktime-13)));
            } else return(0); //LogPrintf("no utxos eligible for staking\n");         
        }
        ...
        else if ( nHeight > 1 && ASSETCHAINS_SYMBOL[0] != 0 && (ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 || ASSETCHAINS_SCRIPTPUB.size() > 1) && (ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0)  && (commission= komodo_commission((CBlock*)&pblocktemplate->block,(int32_t)nHeight)) != 0 )
        {
            ...
            else
            {
                ...
                for (i=0; i<33; i++)
                {
                    ...
                    //LogPrintf("%02x",ptr[i+1]);
                }
                ptr[34] = OP_CHECKSIG;
                //LogPrintf(" set ASSETCHAINS_OVERRIDE_PUBKEY33 into vout[1]\n");
            }
    ...
    try {
        while (true)
        {
            ...
            if (vNodes.empty() && chainparams.MiningRequiresPeers())
            {
                ...
                {
                    LogPrintf("no nodes, attempting reconnect\n");
                    continue;
                }
            }
            ...
            {
                LogPrintf("timeout, retrying\n");
                continue;
            }

            if ( pindexPrev != chainActive.LastTip() )
            {
                LogPrintf("Block %d added to chain\n", chainActive.LastTip()->GetHeight());
                MilliSleep(250);
                continue;
            }
            ...
            LogPrintf("Found block %d \n", Mining_height );
            LogPrintf("staking reward %.8f %s!\n", (double)subsidy / (double)COIN, ASSETCHAINS_SYMBOL);
            arith_uint256 post;
            ...
            if (unlockTime > Mining_height && subsidy >= ASSETCHAINS_TIMELOCKGTE)
                LogPrintf("- timelocked until block %i\n", unlockTime);
            else
                LogPrintf("\n");
...
#ifdef _WIN32
                LogPrintf("%llu mega hashes complete - working\n", (ASSETCHAINS_NONCEMASK[ASSETCHAINS_ALGO] + 1) / 1048576);
#else
                LogPrintf("%lu mega hashes complete - working\n", (ASSETCHAINS_NONCEMASK[ASSETCHAINS_ALGO] + 1) / 1048576);
#endif
                break;
...
//                                                                       [&m_cs, &cancelSolver](const uint256& hashNewTip) mutable {
    ...
    try {
        ...
        while (true)
        {
            ...
            {
                //LogPrintf("%s create new block ht.%d\n",ASSETCHAINS_SYMBOL,Mining_height);
                //sleep(3);
            }
            ...
            if ( ASSETCHAINS_SYMBOL[0] == 0 && notaryid >= 0 )
            {
                ...
                if ( (Mining_height >= 235300 && Mining_height < 236000) || (Mining_height % KOMODO_ELECTION_GAP) > 64 || (Mining_height % KOMODO_ELECTION_GAP) == 0 || Mining_height > 1000000 )
                {
                    ...
                    if ( nonzpkeys > 0 )
                    {
                        ...
                        if ( j == 65 )
                            KOMODO_LASTMINED = 0;
                    } else LogPrintf("no nonz pubkeys\n");
            ...
            while (true)
            {
                ...
                (std::vector<unsigned char> soln) {
                    ...
                    LogPrintf(" mined ");
                    for (z=31; z>=16; z--)
                        LogPrintf("%02x",((uint8_t *)&HASHTarget)[z]);
                    LogPrintf(" hashTarget ");
                    for (z=31; z>=16; z--)
                        LogPrintf("%02x",((uint8_t *)&HASHTarget_POW)[z]);
                    LogPrintf(" POW\n");*/
                    if ( h > hashTarget )
                    {
                    ...
                    {
                        if ( 0 && ASSETCHAINS_SYMBOL[0] != 0 )
                            LogPrintf("timeout, break\n");
                        break;
                    }
                    if ( pindexPrev != chainActive.LastTip() )
                    {
                        if ( 0 && ASSETCHAINS_SYMBOL[0] != 0 )
                            LogPrintf("Tip advanced, break\n");
                        break;
                    }
